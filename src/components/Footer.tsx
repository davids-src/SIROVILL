    ...
    <footer className="border-t border-line/50 bg-surface/40">
      <div className="mx-auto max-w-site px-6 py-16">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3">
              <li><FooterLink to="/szolgaltatasok">Szolgáltatások áttekintő</FooterLink></li>
              <li><FooterLink to="/szolgaltatasok/vilagitas-korszerusites">Világítás korszerűsítés</FooterLink></li>
              <li><FooterLink to="/szolgaltatasok/aramkor-bovites-elosztoszekreny">Áramkör-bővítés, elosztószekrény</FooterLink></li>
              <li><FooterLink to="/szolgaltatasok/hibaelharitas">Hibaelhárítás</FooterLink></li>
              <li><FooterLink to="/szolgaltatasok/kabelezes-epitkezeskor">Kábelezés építkezéskor</FooterLink></li>
              <li><FooterLink to="/szolgaltatasok/okosotthon-vezerles">Okosotthon-vezérlés</FooterLink></li>
            </ul>
          </div>
        ...
        <div className="mt-12 grid grid-cols-2 gap-6 border-t border-line/50 pt-7 text-sm text-muted sm:grid-cols-4">
          {NAV.map((n) => (
            <Link key={n.href} to={n.href} className="hover:text-ink transition-colors duration-150">
              {n.label}
            </Link>
          ))}
        </div>
...
function FooterLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link to={to} className="text-sm text-muted transition-colors duration-150 hover:text-ink">
      {children}
    </Link>
  );
}
